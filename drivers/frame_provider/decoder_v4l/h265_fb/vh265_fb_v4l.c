	...
	struct PIC_s *pic)
{
	...
	if (hevc->mmu_enable
		...
		if (hevc->front_back_mode == 1 ||
			hevc->front_back_mode == 3)
			decoder_mmu_box_free_idx(ibuf->mmu_box_1, pic->index);
#endif
	}
	pic->scatter_alloc = 0;
}
...
{
	if ((dw == 0x10) || hevc->interlace_flag ||
	...
	struct vdec_s *vdec,int from)
{
	...
	if (hevc->is_swap &&
		...
		if (
			...
			hevc->front_back_mode != 3 &&
#endif
			hevc->mmu_enable && ((get_double_write_mode(hevc) & 0x10) == 0)) {
